...
app.post("/compose", function (req, res) {
  ...
  newPost.save().then(function () { /*console.log("Added new post to database")*/ }).catch(function (e) { console.log(e.message) });

  res.redirect("/");
});

app.get("/post/:postTitle", function (req, res) {
  let link = _.lowerCase(req.params.postTitle);

  Post.find({}).insensitiveNameFind(link)
...
app.post("/delete", function (req, res) {
  ...
  Post.find({}).insensitiveNameFind(title)
    .then(function (results) {
      if (results.length !== 0) {
        for(const result of results){
          Post.deleteOne({ title: result.title }).catch(function (e) { console.log(e.message) });
        }
      }
      res.redirect("/");
    })
